...
with col2:
    churn_rate = df_view["Churn"].mean(
    ) if "Churn" in df_view.columns else None
    st.metric("Overall churn rate",
              f"{churn_rate:.2%}" if churn_rate is not None else "N/A")
...
st.dataframe(
    filtered.sort_values("ChurnProbability", ascending=False)[
        show_cols].head(200),
...
if "customerID" in df_view.columns:
    selected_id = st.selectbox(
        "Select customerID",
        options=df_view["customerID"].astype(str).unique()
    )

    # Find the row index for that customer
    ...
    try:
        ...
        x_row = X_enc.loc[idx]  # <-- IMPORTANT: use the same idx
        ...
        if len(drivers) == 0:
            ...
            st.dataframe(drivers.rename("Coefficient").to_frame(),
                         use_container_width=True)
    ...
    st.info("customerID column not found in dataset. Add it to Telco_processed.csv for UI selection.")
